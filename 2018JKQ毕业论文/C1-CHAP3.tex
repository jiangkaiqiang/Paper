...
\begin{figure}[htbp]
\centering{
		\subfigure[固定步长算法的时间自动机模型]{\includegraphics[width=2.0in,height=1.3in]{fig/3/fixedstep_master.png}
			\label{ta_fixedstep}}
		\hfil
		\subfigure[可回滚算法的时间自动机模型]{\includegraphics[width=3.0in,height=1.6in]{fig/3/rollback_master.png}
			\label{ta-rollback}}	
		\subfigure[可预测步长算法的时间自动机模型]{\includegraphics[width=5.0in,height=2.3in]{fig/3/pma_master.png}
			\label{ta-pre}}		
...
\begin{figure}[htbp]
	\centering	{\includegraphics[width=5.0in,height=3.5in]{fig/3/AD.jpg}}
...
\begin{figure}[htbp]
\centering{
		\subfigure[情形1]{\includegraphics[width=5.0in,height=1.1in]{fig/3/CD1.png}
			\label{cd1}}
		\hfil
		\subfigure[情形2]{\includegraphics[width=5.0in,height=1.1in]{fig/3/CD2.png}
			\label{cd2}}
		\hfil
		\subfigure[情形3]{\includegraphics[width=5.0in,height=1.1in]{fig/3/CD3.png}
			\label{cd3}}
	\caption{水箱系统的SysML IBD}
	\label{cd}
	}
\end{figure}
在本小节我们用SysML BDD描述了系统组件的结构，并用SysML的IBD描述了各个组件之间的连接关系。在下一个小节中，我们将基于FMI标准，根据BDD生成各个组件对应的FMU，并根据SysML IBD中描述的关联关系来生成FMU之间的接口配置文件。
...
\begin{figure}[htbp]
\centering{
		\subfigure[FMU\_controller 的时间自动机模型]{\includegraphics[width=2.5in,height=2.0in]{fig/3/2signal_controller.png}
			\label{tk_controller}}
		\hfil
		\subfigure[FMU\_valve 的时间自动机模型]{\includegraphics[width=2.5in,height=2.0in]{fig/3/2signal_v.png}
			\label{tk_v}}
			
	    \subfigure[FMU\_WaterTank1 的时间自动机模型]{\includegraphics[width=2.2in,height=2.0in]{fig/3/2signal_wt1.png}
			\label{tk_wt1}}
		\hfil
		 \subfigure[可回滚主算法的时间自动机模型]{\includegraphics[width=2.8in,height=2.0in]{fig/3/2signal_master.png}
			\label{tk_ma}}		
...
\begin{figure}[htbp]
\centering{
		\subfigure[部分迹]{\includegraphics[width=2.5in,height=3.5in]{fig/3/trs.png}
			\label{trs}}
		\hfil
		\subfigure[执行序列图]{\includegraphics[width=2.5in,height=3.5in]{fig/3/seq.png}
			\label{seq}}
	\caption{时间自动机网络在UPPAAL中的执行序列。}
	\label{trs-seq}
	}
\end{figure}
...
图\ref{trs-seq}为该时间自动机网络模型在UPPAAL中的执行片段，我们可以看到$valve$首先发送了信号$w$ 来与$WaterTank1$进行数据交换，然后$WaterTank1$ 到达了 $dostep$状态，之后主算法广播  $rollback$信号使得FMU到达$reset$状态，最后主算法发送$continue$信号使得所有的FMU到达$start$状态并开始下一步仿真。通过对执行序列的分析，我们发现该模型可以正确的进行仿真。
...
\begin{figure}[htbp]
\centering{
		\subfigure[FMU\_controller 的时间自动机模型]{\includegraphics[width=2.7in,height=2.0in]{fig/3/2signal_cycle_controller.png}
			\label{tk2_controller}}
		\hfil
		\subfigure[FMU\_WaterTank1 的时间自动机模型]{\includegraphics[width=2.3in,height=2.0in]{fig/3/2signal_cycle_wt1.png}
			\label{tk2_v}}		
	\caption{情形2中修改之后的时间自动机}
	\label{tk-arch2}
	}
\end{figure}
\begin{figure}[htbp]
	\centering	{\includegraphics[width=5.0in,height=2.0in]{fig/3/4signal_wt2.png}}
