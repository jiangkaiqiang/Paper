...
Then users can verify the property with DAL-SMC and generate a bar graph as shown in Figure \ref{ui_dsmc}. Users can observe the distribution of traces more obviously with the bar graph.
\begin{figure}[htbp]
	\centering
...
\begin{figure}[htbp]
	\centering
	{\includegraphics[width=3.0in,height=1.2in]{fig/trace-slave.png}}
	%\vspace{0.10in}
	\caption{Trace number of each slave}
   \label{trace-slave}
\end{figure}
The evaluation result is in [0.52707,0.626969] with half-interval size of $\pm0.05$ and interval coverage coefficient of 95\%. Further, we plot the number of traces and satisfying traces of each slave in Figure \ref{trace-slave}. It shows that the slaves generate same number of traces, however, the number of satisfying traces are not equal. Besides, in order to compare the probability partition between DAL-SMC and DAL-SMC opt, we plot the probability of each sub-space as shown in Figure \ref{BIE-pro}. Figure \ref{pronor} and Figure \ref{proopt} are the probability of each sub-space in DAL-SMC and DAL-SMC opt. respectively. We find that the probability distribution of DAL-SMC opt. is more evenly than that of DAL-SMC, therefore, the parameter optimization is effective. In the next subsections, we will use two complex CPS benchmarks to compare the performance of algorithms.
\begin{figure*}[htbp]
\centering{
		\subfigure[DAL-SMC]{\includegraphics[width=2.5in,height=1.4in]{fig/pro.png}
			\label{pronor}}
		\hfil
		\subfigure[DAL-SMC opt]{\includegraphics[width=2.5in,height=1.4in]{fig/pro-opt.png}
			\label{proopt}}
...
\begin{figure*}[htbp]
\centering{
		\subfigure[Trace number of each algorithm]{\includegraphics[width=1.8in,height=1.7in]{fig/sb-trace.png}
			\label{trace_sb}}
		\hfil
		\subfigure[Verification time of each algorithm]{\includegraphics[width=1.8in,height=1.7in]{fig/sb-time.png}
			\label{time_sb}}
		\hfil
		\subfigure[Statistical error of each algorithm]{\includegraphics[width=1.8in,height=1.7in]{fig/sb-error.png}
			\label{error_sb}}
...
\centering{
		\subfigure[Trace number of each algorithm]{\includegraphics[width=1.8in,height=1.7in]{fig/ro-trace.png}
			\label{trace_ro}}
		\hfil
		\subfigure[Verification time of each algorithm]{\includegraphics[width=1.8in,height=1.7in]{fig/ro-time.png}
			\label{time_ro}}
		\hfil
		\subfigure[Statistical error of each algorithm]{\includegraphics[width=1.8in,height=1.7in]{fig/ro-error.png}
			\label{error_ro}}
...
\begin{table}[t]
	\renewcommand{\arraystretch}{1.2}
	\caption{Properties in robots path planning}
	\label{tb:robot}
	\centering
	\begin{tabular}{c c l}
		\hline
